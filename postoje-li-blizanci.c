...
int postoje_li_blizanci(Node *node) {

    // osnovni slučaj
    // nema stabla ili smo na kraju
    if(node == NULL
        || (node->left == NULL && node->right == NULL)) return 0;

    // napredak
        // postoje blizanci (prvo provjera postoje li djeca!)
    if(node->left && node->right &&
        node->left->key == node->right->key) return 1;
        // napreduj kroz l i d podstablo
    return (postoje_li_blizanci(node->left) || postoje_li_blizanci(node->right));
}
...
int main () {
    ...
    do {
        menu_choice = 0;
        //DEBUG("\n1 Da li u stablu postoji barem jedan cvor s dvoje djece koja imaju isti kljuc (postoje_li_blizanci)");
        //DEBUG("\n2 Kreiraj stablo \n3 Ispis \n4 Izlaz\n");
        scanf("%d", &menu_choice);
        switch (menu_choice) {
            case 1:
                printf("%c\n", (postoje_li_blizanci(root) ? (char)'Y' : (char)'N'));
                break;
            case 2:
