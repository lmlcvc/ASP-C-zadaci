...
int suma(Node *node, int val) {
    // osnovni slučaj - prazni node
    if(!node) return 0;

    // napredak
    // sumu množimo s 10 i dodajemo joj tren. vrijednost
    val = val*10 + node->key;
    // došli smo u list - vraćamo vrijednost tog puta
    if(!(node->left) && !(node->right)) return val;
    // poziv f-je u roditeljima za lijevo i desno podstablo
    // zbrajanje tih dvaju vrijednosti
