...
int broj_blizanaca(Node *node) {
    int bliz = 0; // pretpostavljamo da nisu

        // osnovni uvjet - null node
    if(!node) return 0;

        // napredak
    // pribrojiti rezultat za trenutni čvor - postoje i isti su
    // i pozvati rekurziju na lijevo i desno podstablo
    bliz = (node->left && node->right && node->left->key == node->right->key);
    return broj_blizanaca(node->left) + broj_blizanaca(node->right) + bliz;
}
