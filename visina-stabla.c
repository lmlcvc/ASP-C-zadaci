...
int visina(Node *node) {
    ...
    // osnovni slučaj
    // stablo je prazno ili trenutni čvor nema djece
    if(node == NULL
        || (node->left == NULL && node->right == NULL)) return 0;

    // napredak
    // računanje visine lijevog i desnog podstabla
    int l = visina(node->left) + 1;
    int r = visina(node->right) + 1;

    // visina stabla je visina većeg podstabla
    return l > r ? l : r ;
}
